...
export default function CreateLocationModal({ organizationId, onClose, onSuccess }: { organizationId: string, onClose: () => void, onSuccess: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null)
    try {
      const res = await eventsApi.createLocation(formData)
      if (res.success) {
        onSuccess()
      } else {
        throw new Error(res.error || 'Failed to register location')
      }
    } catch (err: any) {
      ...
      <div className="bg-white rounded-[40px] w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
        <div className="p-8 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <div className="flex items-center space-x-3">
             ...
             <div>
                <h2 className="text-xl font-black text-gray-900 italic tracking-tighter">Locate Venue.</h2>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none">New spatial resource registration</p>
             </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Venue Identity Name</label>
              <input
                required
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Physical Address / Directions</label>
              <textarea
                className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl text-[11px] font-bold focus:ring-4 focus:ring-red-500/5 outline-none transition-all focus:bg-white focus:border-red-500 h-24 resize-none"
                placeholder="Operational address for participants..."
          ...
          >
             {loading ? <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" /> : (
               <>
                 <span>Pin New Spatial Identity</span>
