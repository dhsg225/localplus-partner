'use client'

import { motion } from "framer-motion"
import Link from "next/link"
import { usePathname } from "next/navigation"

export default function AnswerEngineNavbar() {
  const pathname = usePathname()

  const links = [
    { label: "Consumers", href: "/answer-engine" },
    { label: "Businesses", href: "/answer-engine/businesses" },
    { label: "How It Works", href: "/answer-engine/tech" }
  ]

  const placeholders = [
    { label: "Platform" },
    { label: "Solutions" },
    { label: "Pricing" }
  ]

  return (
    <nav className="fixed top-6 left-1/2 -translate-x-1/2 w-[calc(100%-3rem)] max-w-7xl h-16 px-8 flex justify-between items-center z-[110] bg-white/70 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl transition-all duration-300">
        <div className="flex items-center space-x-3">
           <a href="/" className="flex items-center space-x-3 group">
              <div className="w-8 h-8 rounded-lg bg-gray-900 flex items-center justify-center font-black text-white text-[10px] shadow-lg group-hover:scale-110 transition-transform">LP</div>
              <span className="text-sm font-black tracking-tight text-gray-900 uppercase">LocalPlus</span>
           </a>
        </div>
        
        {/* Scenario Pill (Screenshot 2 Style) */}
        <div className="hidden lg:flex items-center h-full">
            <div className="bg-gray-100/50 p-1.5 rounded-full flex items-center space-x-1">
               {links.map((link) => {
                 const isActive = pathname === link.href
                 return (
                   <Link 
                     key={link.label}
                     href={link.href} 
                     className={`px-6 py-2 rounded-full text-[10px] font-black transition-all uppercase tracking-widest relative ${
                       isActive ? 'bg-gray-900 text-white shadow-xl translate-y-[0px]' : 'text-gray-400 hover:text-gray-900'
                     }`}
                   >
                     {link.label}
                   </Link>
                 )
               })}
               <div className="h-4 w-[1px] bg-gray-200 mx-2" />
               <div className="flex items-center space-x-4 pr-4">
                  {placeholders.map((p) => (
                    <div 
                      key={p.label}
                      className="px-4 py-2 opacity-20 hover:opacity-100 transition-opacity cursor-not-allowed"
                    >
                      <span className="text-[11px] font-black uppercase tracking-[0.3em] whitespace-nowrap text-gray-900">
                         {p.label}
                      </span>
                    </div>
                  ))}
              </div>
            </div>
        </div>

        <div className="flex items-center space-x-4">
           <a 
              href="/login" 
              className="px-6 py-2.5 bg-gray-900 text-white rounded-full text-xs font-black hover:bg-black transition-all shadow-xl hover:shadow-2xl active:scale-95 uppercase tracking-widest text-center"
           >
              Login
           </a>
        </div>
    </nav>
  )
}
